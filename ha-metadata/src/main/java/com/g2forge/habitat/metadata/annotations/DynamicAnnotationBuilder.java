...
public class DynamicAnnotationBuilder<A extends Annotation> implements IBuilder<A> {
	@Data
	protected static class Handler implements InvocationHandler {
		...
		protected boolean isEquals(Object that) throws Exception {
			...
			if (that instanceof Proxy) {
				...
				if (thatInvocationHandler instanceof Handler) return equals(thatInvocationHandler);
			}
	...
	public <T> DynamicAnnotationBuilder<A> add(ISerializableFunction1<A, T> name, T value) {
		return add(name.asMethodAnalyzer().getMethod(), value);
	}

	public DynamicAnnotationBuilder<A> add(Method method, Object value) {
		if (values.containsKey(method.getName())) throw new IllegalStateException("Value for " + method.getName() + " was already set");
		values.put(method.getName(), value);
		return this;
	}

	public DynamicAnnotationBuilder<A> add(String name, Object value) {
	...
	public A build() {
		return (A) Proxy.newProxyInstance(annotationType.getClassLoader(), new Class[] { annotationType }, new Handler(annotationType, values));
	}
}