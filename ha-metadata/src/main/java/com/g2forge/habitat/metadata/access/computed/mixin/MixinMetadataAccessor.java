...
public class MixinMetadataAccessor implements IApplicableMetadataAccessor {
	...
	public <T> IPredicate<T> bind(ISubject subject, IPredicateType<T> predicateType) {
		if (!isApplicable(subject.getType(), predicateType)) throw new IllegalArgumentException(String.format("This combination of subject (%1$s) and predicate type (%2$s) is not acceptable to this accessor!", subject, predicateType));
	...
	public boolean isApplicable(ISubjectType subjectType, IPredicateType<?> predicateType) {
		return getSubjectType().test(subjectType) && getPredicateType().test(predicateType);
	}
}
